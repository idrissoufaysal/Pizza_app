...
export default function Nav() {
  ...
  const totalPizzaCount = pizzas?.reduce(
    ...
    <nav className="fixed top-0 z-10 flex justify-between w-full shadow-md h-[50px] items-center bg-white  p-5">
      ...
      <ul className="hidden md:flex gap-3 justify-center items-center">
        {menu.map((menu, index) => (
          <div
            key={index}
            className="flex items-center gap-1 cursor-pointer hover:text-orange-500 hover:underline"
          >
            <span>{<menu.icon />}</span>
            <span>{menu.name} </span>
          </div>
      ...
      <button
        ...
        className="flex gap-2 items-center justify-center h-8 w-14 bg-orange-700 rounded-md hover:bg-orange-500"
      >
        <IoCart className="text-white" />
        <span className="text-white">{totalPizzaCount}</span>
      </button>
      {isMenuOpen && (
        <CartModal isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} />
      )}
    </nav>
  );
}
